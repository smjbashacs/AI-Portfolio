<div style="display: flex; justify-content: space-between; width: 100%; padding: 0 20px; gap: 10px; margin-bottom: 18px; background-color: #B4CBF0; border-bottom: 1px solid #ccc; border-radius: 8px">
    <a href="#ml-projects" style="color: black; font-family: monospace; padding: 10px 20px; cursor: pointer; font-weight: bold;">Machine Learning</a>
    <a href="#nlp-projects" style="color: black; font-family: monospace; padding: 10px 20px; cursor: pointer; font-weight: bold;">NLP/GenAI</a>
    <a href="#dl-projects" style="color: black; font-family: monospace; padding: 10px 20px; cursor: pointer; font-weight: bold;">Deep Learning</a>
    <a href="#ml-scratch" style="color: black; font-family: monospace; padding: 10px 20px; cursor: pointer; font-weight: bold;">Build from scratch</a>
</div>

<hr />

<h1 style="text-align: center; color:#764cc1" id="ml-projects">Machine Learning Projects</h1>
<!-- <div style="background-color: #E8E8FA; padding: 20px 20px 0 20px; margin-bottom: 40px; border-radius: 8px; border: 1px solid black;"> -->
  
<div style="width: 900; height: 300px; padding: 10px; display: block; background-color: #FAFAFC; border-radius: 8px; overflow-y: auto; margin-bottom: 20px; border: 1px solid #E2E2E3; box-shadow: 0 0 5px 3px #E8E8FA"> 
    <h2 style="color: #00949A; font-family: Arial">Classification of IRIS Data</h2>
    <div style="color: black;  font-family: Arial; font-size: 18px; line-height: 1.5; font-weight: 500">
      This project deals with using ML algorithms to classify data points.
      We will be using Decision trees, Linear Regression, and Random forests.
      <p>Deep learning algorithms</p>
      <p>Natural Language processing</p>
      <p>Even more content...</p>
    </div>
</div>

<div style="width: 900; height: 300px; padding: 10px; display: block; background-color: #FAFAFC; border-radius: 8px; overflow-y: auto; margin-bottom: 20px; border: 1px solid #E2E2E3; box-shadow: 0 0 5px 3px #E8E8FA"> 
    <h2 style="color: #00949A; font-family: Roboto Mono;">Regression - real estate pricing</h2>
    <div style="color: black;  font-family: IBM Plex Mono; font-size: 18px; line-height: 1.5; font-weight: 500;">
      This project deals with using ML algorithms to classify data points.
      We will be using Decision trees, Linear Regression, and Random forests.
      <p>Deep learning algorithms</p>
      <p>Natural Language processing</p>
      <p>Even more content...</p>
    </div>
</div>
  
<!-- </div> -->

<h1 style="text-align: center; color:#764cc1" id="nlp-projects">NLP Projects</h1>

<div style="width: 900; height: 300px; padding: 10px; display: block; background-color: #FAFAFC; border-radius: 8px; overflow-y: auto; margin-bottom: 20px; border: 1px solid #E2E2E3; box-shadow: 0 0 5px 3px #E8E8FA"> 
    <h2 style="color: #00949A; font-family: Arial">Sentiment analysis</h2>
    <div style="color: black; font-family: Open Sans; font-size: 18px; line-height: 1.5; font-weight: 500;">
      This project deals with using ML algorithms to classify data points.
      We will be using Decision trees, Linear Regression, and Random forests.
      <p>Deep learning algorithms</p>
      <p>Natural Language processing</p>
      <p>Even more content...</p>
    </div>
</div>

<h1 style="text-align: center; color:#764cc1" id="dl-projects">Deep Learning Projects</h1>

<div style="width: 900; height: 300px; padding: 10px; display: block; background-color: #FAFAFC; border-radius: 8px; overflow-y: auto; margin-bottom: 20px; border: 1px solid #E2E2E3; box-shadow: 0 0 5px 3px #E8E8FA"> 
    <h2 style="color: #00949A; font-family: Arial">Image detection</h2>
    <div style="color: black; font-family: Montserrat; font-size: 18px; line-height: 1.5; font-weight: 500;">
      This project deals with using ML algorithms to classify data points.
      We will be using Decision trees, Linear Regression, and Random forests.
      <p>Deep learning algorithms</p>
      <p>Natural Language processing</p>
      <p>Even more content...</p>
      <p>Deep learning algorithms</p>
      <p>Natural Language processing</p>
      <p>Even more content...</p>
    </div>
</div>

